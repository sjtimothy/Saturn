...
public partial class SgtTerrain : MonoBehaviour
{
	...
	[System.NonSerialized]
	private bool updatePatchesCalled;
	...
	public Vector3 GetSurfacePositionLocal(Vector3 localPosition, float offset = 0.0f)
	{
		...
		displacement = Radius + Height * displacement + offset;

		return localPosition.normalized * displacement;
	}
	...
	private void UpdatePatches()
	{
		...
		if (patchIndex < patches.Count)
		{
			...
			while (patchIndex < patches.Count)
			{
				...
				if (budgetWatch.Elapsed.TotalSeconds >= Budget)
				{
					break;
				}
			}
